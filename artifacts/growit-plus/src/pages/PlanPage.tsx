...
import { Download, Share2, RotateCcw, MapPin, Calendar, Info, Sparkles, Cpu, Wand2, Pencil, X, Search, RefreshCw, Loader2 } from "lucide-react";
...
import { downloadPlanAsPdf } from "@/utils/pdf-export";
...
export default function PlanPage({ plan, onStartOver }: PlanPageProps) {
  ...
  // ── Download plan as branded PDF ─────────────────────────────────────────
  const downloadPlan = async () => {
    if (downloading) return;
    setDownloadError(false);
    setDownloading(true);
    try {
      // Defer to next tick so the spinner can paint before the (sync) PDF build
      await new Promise(r => setTimeout(r, 30));
      downloadPlanAsPdf(localPlan);
    } catch (err) {
  ...
  return (
    <div className="w-full flex-1 flex flex-col h-full bg-cream animate-in fade-in duration-500 overflow-hidden">
      ...
      {/* PDF download error toast */}
      {downloadError && (
        <div className="fixed top-4 left-1/2 -translate-x-1/2 bg-terracotta text-cream px-4 py-2 rounded-xl text-sm font-medium shadow-lg animate-in fade-in z-50">
          PDF download is unavailable right now. Please try again.
        </div>
      )}
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <div className="ml-auto flex gap-2">
            <button
              ...
              className="w-9 h-9 rounded-full bg-cream-light border border-cream-dark flex items-center justify-center text-forest hover:bg-cream-dark transition-colors disabled:opacity-60 disabled:cursor-wait"
              data-testid="btn-download"
              aria-label={downloading ? "Generating PDF" : "Download Plan"}
              title={downloading ? "Generating PDF…" : "Download Plan"}>
              {downloading
